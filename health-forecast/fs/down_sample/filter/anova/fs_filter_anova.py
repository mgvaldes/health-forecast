...
for i in range(0, num_experiments):
    ...
    for C in C_OPTIONS:
        ...
        for train_index, test_index in skf.split(X_train, y_train):
            ...
            print()
            # Perform FS
            anova_k_best_result = SelectPercentile(f_classif, percentile=10)
            anova_k_best_result.fit(X_train[train_index, :], y_train[train_index])

            print("Transforming X_train of fold " + str(j) + " with selected features")
            print()
            # Use only selected features of training set
            reduced_X_train = anova_k_best_result.transform(X_train[train_index, :])

            print("Selected " + str(reduced_X_train.shape[1]) + " features")
            print()

            linear_svm = LinearSVC(C=C, penalty='l1', random_state=seeds[i], dual=False)
            print("Training LinearSVC with reduced X_train of fold " + str(j))
            print()
            ...
            y_pred = linear_svm.predict((X_train[test_index, :])[:, anova_k_best_result.get_support()])

            linear_svm_precision_recall_fscore_support = precision_recall_fscore_support(y_train[test_index], y_pred)
            pos_f1 = linear_svm_precision_recall_fscore_support[2][0]
            neg_f1 = linear_svm_precision_recall_fscore_support[2][1]

            F1s.append(np.mean([pos_f1, neg_f1]))
    ...
    best_C = list(C_gridsearch_results.keys())[list(C_gridsearch_results.values()).index(max(list(C_gridsearch_results.values())))]
    print(best_C)
    print()

    print("Performing FS over X_train")
    print()
    anova_k_best_result = SelectPercentile(f_classif, percentile=10)
    anova_k_best_result.fit(X_train, y_train)

    experiment_results['fs'] = anova_k_best_result

    selected_features = np.zeros(X_train.shape[1])
    selected_features[anova_k_best_result.get_support()] = 1

    feature_ranking[:, i] = selected_features

    print("Transforming X_train with selected features")
    print()
    # Use only selected features of training set
    reduced_X_train = anova_k_best_result.transform(X_train)

    linear_svm = LinearSVC(C=C, penalty='l1', random_state=seeds[i], dual=False)
    print("Training final LinearSVC model with best C parameter and reduced X_train")
    print()
    linear_svm.fit(reduced_X_train, y_train)

    experiment_results['model'] = linear_svm

    print("Predicting y_test with reduced X_test")
    # Use only selected features of test set
    y_pred = linear_svm.predict(X_test[:, anova_k_best_result.get_support()])

    print("Decision function:")
    y_score = linear_svm.decision_function(X_test[:, anova_k_best_result.get_support()])
    ...
    plot_confusion_matrix(linear_svm_confusion_matrix, classes=["Positive", "Negative"],
                          filename=os.getcwd() + '/classifiers/linear_svm/experiment_' + str(i) + '_down_sample_confusion_matrix.png')
    ...
    print("AUC:")
    linear_svm_auc = roc_auc_score(y_test, y_score)
    ...
    plot_roc(fpr, tpr, pos_auc, "Positive ROC", filename=os.getcwd() + '/classifiers/linear_svm/experiment_' + str(i) + '_down_sample_pos_roc.png')

    fnr, tnr, thresholds = roc_curve(y_test, y_score, pos_label=1)
    neg_auc = auc(fnr, tnr)
    experiment_results['fnr'] = fnr
    experiment_results['tnr'] = tnr
    experiment_results['neg_auc'] = neg_auc
    plot_roc(fnr, tnr, neg_auc, "Negative ROC", filename=os.getcwd() + '/classifiers/linear_svm/experiment_' + str(i) + '_down_sample_neg_roc.png')
...
save_object(experiments_results, os.getcwd() + '/classifiers/linear_svm/linear_svm_results.pkl')
save_object(feature_ranking, os.getcwd() + '/classifiers/linear_svm/feature_ranking.pkl')