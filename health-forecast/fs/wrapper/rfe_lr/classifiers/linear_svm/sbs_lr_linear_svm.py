...
rfe_ranking = np.zeros((len(variable_names), num_experiments))
...
for i in range(0, num_experiments):
    ...
    for C in C_OPTIONS:
        ...
        for train_index, test_index in skf.split(X_train, y_train):
            ...
            print()
            # Perform FS
            lr = LogisticRegression(random_state=seeds[i], class_weight="balanced")

            sbs = SFS(lr, k_features=round(0.1 * X_train.shape[1]), forward=False, floating=False, scoring='f1_weighted',
                      cv=0, n_jobs=-1)
            sbs.fit(X_train[train_index, :], y_train[train_index])

            print("Transforming X_train of fold " + str(j) + " with selected features")
            print()
            # Use only selected features of training set
            reduced_X_train = sbs.transform(X_train[train_index, :])
            ...
            y_pred = linear_svm.predict((X_train[test_index, :])[:, sbs.k_feature_idx_])

            F1s.append(f1_score(y_train[test_index], y_pred, average='weighted'))

            j += 1

        print("For C=%s --- %s seconds ---" % (C, time.time() - start_time))

        C_gridsearch_results[C] = np.mean(np.array(F1s))

    print("For gridsearch over C --- %s seconds ---" % (time.time() - start_time))
    ...
    sbs = SFS(lr, k_features=round(0.1 * X_train.shape[1]), forward=False, floating=False, scoring='f1_weighted', cv=0,
              n_jobs=-1)
    sbs.fit(X_train, y_train)

    linear_svm = SVC(kernel='linear', random_state=seeds[i], probability=True, class_weight='balanced', C=best_C)

    experiment_results['fs'] = sbs

    selected_features = np.zeros(X_train.shape[1])
    selected_features[sbs.k_feature_idx_] = 1

    feature_ranking[:, i] = selected_features

    linear_svm.fit(X_train[:, sbs.k_feature_idx_], y_train)

    experiment_results['model'] = linear_svm

    linear_svm_coefficients = np.zeros(X_train.shape[1])
    linear_svm_coefficients[sbs.k_feature_idx_] = np.absolute(linear_svm.coef_)
    ...
    print()
    # Use only selected features of test set
    y_pred = linear_svm.predict(X_test[:, sbs.k_feature_idx_])
    ...
    print("Probabilities:")
    y_prob = linear_svm.predict_proba(X_test)
...
print("After all experiments --- %s seconds ---" % (time.time() - start_time))
...
features_info = np.array(list(zip(np.repeat('', len(variable_names)), np.repeat(0, len(variable_names)), np.repeat(0, len(variable_names)), np.repeat(0, len(variable_names)), np.repeat(0, len(variable_names)))), dtype=[('names', 'S12'), ('stability', '>i4'), ('performance (F1)', '>f4'), ('linear SVM coefficients', '>f4')])
...
with open(os.getcwd() + '/features_info.csv', 'w') as f:
    w = csv.writer(f)
