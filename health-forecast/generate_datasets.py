...
epidem_data = np.genfromtxt('/home/aegle/Downloads/IMPPC_lung_progres.txt', delimiter=' ')
...
with open('/home/aegle/Downloads/IMPPC_lung_progres.txt', 'r') as csvfile:
...
for i in range(2, 23):
    print("Loading genomic data from chr", str(i))

    genomic_data = np.genfromtxt('/home/aegle/Downloads/all_genome/chr' + str(i) + '/maf_filter/chr' + str(i) + '_imputed_maf001.raw', delimiter=' ')
    ...
    with open('/home/aegle/Downloads/all_genome/chr' + str(i) + '/maf_filter/chr' + str(i) + '_imputed_maf001.raw', 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=' ')
        for row in reader:
    ...
    chr_dir = os.getcwd() + '/datasets/lung_cancer/chr' + str(i)

    if not os.path.exists(chr_dir):
        os.makedirs(chr_dir)

    dataset_type_dir = chr_dir + '/genomic'

    if not os.path.exists(dataset_type_dir):
        os.makedirs(dataset_type_dir)

    genomic_dataset = np.zeros((genomic_data_n_rows, (genomic_data_n_cols + 1)))
    genomic_dataset[:, 0] = epidem_data[:, -1] - 1
    genomic_dataset[:, 1:] = genomic_data

    genomic_variable_names = np.append(epidem_data_variable_names[-1], genomic_data_variable_names)

    print("Spliting into train and test for genomic dataset type...")
    train_indexes, test_indexes = list(StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=6547891).split(genomic_dataset[:, 1:], genomic_dataset[:, 0]))[0]

    print("Saving train for genomic dataset type...")
    with open(dataset_type_dir + '/raw_train.csv', 'w') as f:
        w = csv.writer(f)
        w.writerow(genomic_variable_names)
        w.writerows(genomic_dataset[train_indexes, :])

    print("Saving test for genomic dataset type...")
    with open(dataset_type_dir + '/raw_test.csv', 'w') as f:
        w = csv.writer(f)
        w.writerow(genomic_variable_names)
        w.writerows(genomic_dataset[test_indexes, :])

    dataset_type_dir = chr_dir + '/genomic_epidemiological'
    ...
    variable_names = np.append(np.append(epidem_data_variable_names[-1], epidem_data_variable_names[:-1]),
                               genomic_data_variable_names)

    print("Spliting into train and test for genomic_epidemiological dataset type...")
    train_indexes, test_indexes = list(StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=6547891).split(
        complete_dataset[:, 1:], complete_dataset[:, 0]))[0]

    print("Saving train for genomic_epidemiological dataset type...")
    with open(dataset_type_dir + '/raw_train.csv', 'w') as f:
        w = csv.writer(f)
        w.writerow(variable_names)
        w.writerows(complete_dataset[train_indexes, :])

    print("Saving test for genomic_epidemiological dataset type...")
    with open(dataset_type_dir + '/raw_test.csv', 'w') as f:
        w = csv.writer(f)
        w.writerow(variable_names)
        w.writerows(complete_dataset[test_indexes, :])
...
#
# reduced_data = np.genfromtxt(os.getcwd() + '/datasets/lung_cancer/' + chr_num + '/complete_dataset_with_pheno.csv', delimiter=',')
...
# with open(os.getcwd() + '/datasets/lung_cancer/' + chr_num + '/complete_dataset_with_pheno.csv', 'r') as csvfile:
#     reader = csv.reader(csvfile, delimiter=',')
#     for row in reader:
#         variable_names = np.array(list(row))
#         break
...
# train_indexes, test_indexes = list(StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=6547891).split(reduced_data[:, 1:], reduced_data[:, 0]))[0]
#
...
# with open(os.getcwd() + '/datasets/lung_cancer/' + chr_num + '/raw_train.csv', 'w') as f:
#     w = csv.writer(f)
#     w.writerow(variable_names)
...
# with open(os.getcwd() + '/datasets/lung_cancer/' + chr_num + '/raw_test.csv', 'w') as f:
#     w = csv.writer(f)
#     w.writerow(variable_names)
...
# print("Loading data...")
# print()
# reduced_data = np.genfromtxt(os.getcwd() + '/datasets/lung_cancer/' + chr_num + '/raw_train.csv', delimiter=',')
# reduced_data = reduced_data[1:, :]
#
# with open(os.getcwd() + '/datasets/lung_cancer/' + chr_num + '/complete_dataset_with_pheno.csv', 'r') as csvfile:
#     reader = csv.reader(csvfile, delimiter=',')
...
#     seed = seeds[i]
#
#     print("Shuffling train data for experiment", str(i))
#     train_dataset = resample(reduced_data, replace=False, random_state=seed)
#
#     train_dataset_name = os.getcwd() + '/datasets/lung_cancer/experiment_' + str(i) + '_raw_train.csv'
#
#     print("Saving experiment", str(i), "data")
#     print()
